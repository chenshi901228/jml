function init(i) {
    $(".brandInfo").html(`
        ${
        dataInfo.brandInfo.content.slice(i, i + 7).map(item => {
            return `
                <div class="pro-normal ${(item.id + 1) % 7 == 0 ? "big" : ""}" data-index=${item.id} data-ify="brandInfo">
                    <div>
...
const pageAll = Math.ceil(dataInfo.brandInfo.content.length / 7)
$(function () {
    init(0)
    for (let i = 0; i < pageAll; i++) {
        $(".dynamic-pagination ul").append(`
            <li>
                <a href="javascript:(0);" data-index="${i}">${i + 1}</a>
            </li>
        `)
    }
})
