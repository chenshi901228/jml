const pageAll = Math.ceil(dataInfo.gameDevelopInfo.content.slice(0, 3).length / 7)

function init(i) {
    $(".gameDevelopInfo").html(`
                ${
        dataInfo.gameDevelopInfo.content.slice(i, i + 7).map(item => {
            return `
                        <div class="pro-normal ${(item.id + 1) % 7 == 0 ? "big" : ""}" data-index=${item.id} data-ify="gameDevelopInfo">
                            <div>
